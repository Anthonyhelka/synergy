...
class Api::V1::SummonerController < ApplicationController
  ...
  def create
    ...
    if summoner_info.code == 200
      ...
      summoner.save
      render json: { summoner: summoner, message: {type: "Success", content: "Successfully Updated!"}, valid: true, }
    else
      render json: { summoner: {}, message: {type: "Error", content: "Something went wrong, Please Try Again."}, valid: false }
    end
  end
  ...
  def show
    ...
    if summoner.blank?
      ...
      render json: { summoner: summoner, message: {type: "", content: ""}, valid: true }
    end
  end
end
